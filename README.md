...
> **logfmt の構造表現は平坦化**
>
> logfmt はフラットな `key=value` 形式のため、ネストはドット平坦化（`user.id=42`）、配列は値内 JSON（`tags=["a","b"]`）で表現する。空の struct・配列は展開対象がないが、キーが消えないよう値内 JSON で `meta={}` / `tags=[]` と出力する。標準の logfmt パーサーはこれらを**フラットなキー/文字列としてパースするのみで、構造には復元しない**（ドットを区切りとして展開するかは受信側の集計基盤の設定次第）。構造を完全に保持・復元したい場合は JSON 形式（`.format = .json`）を使う。
...
> **行のアトミック性は writer バッファに依存する**
>
> zlog は 1 行を `writer` へ直接ストリーミングするため、**1 行の長さが `writer` のバッファ容量を超えると行の途中で書き込み（drain）が走る**。そこで書き込みが失敗すると、改行なしの部分行が出力先に残ったまま `WriteFailed` が返る。行が分割されずに出力されること（atomicity）は、`writer` のバッファが想定する最長行以上のときに成立する。属性を多く載せて 1 行が長くなる場合は、それに見合うバッファ（上記の例では `4096` バイト）を `writer` に渡しておく。

ただしデフォルトではロックを持たない。**複数スレッドから同じ `writer`（同一の `*std.Io.Writer`）へロックなしで並行出力すると、内部バッファの状態（書き込み位置）を同時に書き換えるデータ競合となり、未定義動作になる。** これは「1 ログ行が複数の書き込みに分割されて出力が交錯する」という見た目の問題にとどまらず、バッファ破壊やクラッシュを含む。`writer` を複数スレッドで共有するなら、**メモリ安全性（正しさ）のために** `Options.mutex` が必須となる。

並行出力する場合は `Options.mutex` に共有 `std.Io.Mutex` を渡す。zlog が各ログ出力（タイムスタンプの採取〜書き込み〜flush 全体）をロックで囲むため、ログ行が交錯せず、タイムスタンプの採取もロック取得順（＝出力順）に直列化される。`withFields` で派生したロガーにも同じ mutex が引き継がれる。

> **タイムスタンプ値は単調増加とは限らない**
>
> タイムスタンプは実時刻（wall clock, `Clock.real`）で採取するため、NTP 補正やシステム時刻の手動変更により不連続にジャンプ・逆行しうる。mutex 使用時もタイムスタンプの**採取順**は出力順と一致するが、記録される**値**が単調増加する保証はない（後の行が前の行より過去の `time=` を持ちうる）。加えて精度はミリ秒のため、同一ミリ秒内に出力された複数行は同じ `time=` になる。**`time=` を唯一のソートキーにせず**、厳密な順序が必要なら単調なシーケンス番号などを属性に載せる。
